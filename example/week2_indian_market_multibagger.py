...
# ============================================================
# ============================================================
# SEQUENTIAL ANALYST EXECUTION (Avoids Groq Free Tier Hanging)
# ============================================================
...
async def run_analysts(stock):
    ...
    print("\n")
    print("=" * 70)
    print(f"  RUNNING EQUITY EVALUATION — {stock}")
    print("=" * 70)

    # Run sequentially with a tiny cooling sleep window to avoid API rate-limiting hangs
    gov = await run_agent(governance_agent, prompt)
    await asyncio.sleep(2)  # 2-second breathing room for free-tier limits
    
    fin = await run_agent(financials_agent, prompt)
    await asyncio.sleep(2)
    
    macro = await run_agent(macro_agent, prompt)
    await asyncio.sleep(2)
    
    tech = await run_agent(technical_agent, prompt)
...
if __name__ == "__main__":
    ...
    STOCK = "Gravitas"  # Change this to any NSE/BSE listed company
